...
test('Get Pokémon Iron Thorns', async ({ page }) => {
  const response = await page.request.get(
    'https://pokeapi.co/api/v2/pokemon/iron-thorns'
  );
  const pokemon = await response.json();
  console.log(pokemon.abilities[0].ability.name);
...
test('Get Jaw Fossil', async ({ page }) => {
  const response = await page.request.get(
    'https://pokeapi.co/api/v2/item/jaw-fossil'
  );
  const item = await response.json();
  console.log(item.effect_entries[0].effect);
  await expect(item.effect_entries[0].effect).toBe(
...
test('Get Moon Ball', async ({ page }) => {
  const response = await page.request.get(
    'https://pokeapi.co/api/v2/item/moon-ball'
  );
  const item = await response.json();
  console.log(item.effect_entries[0].short_effect);
  await expect(item.effect_entries[0].short_effect).toBe(
...
test('Get Yellow Apricorn', async ({ page }) => {
  const response = await page.request.get(
    'https://pokeapi.co/api/v2/item/yellow-apricorn/'
  );
  const item = await response.json();
  console.log(item.effect_entries[0].effect);
  await expect(item.effect_entries[0].effect).toBe(
    'May be given to Kurt in azalea town to produce a moon ball.'
  );
  console.log(item.effect_entries[0].short_effect);
  await expect(item.effect_entries[0].short_effect).toBe(
...
test('Get Normal type', async ({ page }) => {
  ...
  await expect(type.damage_relations.double_damage_from[0].name).toBe(
    'fighting'
  );
  console.log(
    'Deals half damage to ' + type.damage_relations.half_damage_to[0].name
  );
  await expect(type.damage_relations.half_damage_to[0].name).toBe('rock');
  console.log(
    'Deals half damage to ' + type.damage_relations.half_damage_to[1].name
  );
  await expect(type.damage_relations.half_damage_to[1].name).toBe('steel');
  console.log(
    'Takes no damage from ' + type.damage_relations.no_damage_from[0].name
  );
  await expect(type.damage_relations.no_damage_from[0].name).toBe('ghost');
  console.log(
    'Deals no damage to ' + type.damage_relations.no_damage_to[0].name
  );
...
test('Get Paldea region', async ({ page }) => {
  const response = await page.request.get(
    'https://pokeapi.co/api/v2/region/paldea/'
  );
